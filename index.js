...
/**
 * PrimeTube yt-dlp microservice (auth-wall resilient / cookie-smart)
 * - Parallel client probing (tv/android/ios/web) + tiny merge window
 * - Coalesced in-flight, short-lived caches, bounded concurrency
 * - Cookie handling (PATH / B64_GZ / B64), auto-select best source
 * - Auto-disable weak cookies (small/missing SID family) to avoid auth walls
 * - Cookies used for clients in COOKIE_CLIENTS *only if* cookieGood=true
 * - Per-request overrides: ?nocookie=1  and  ?clients=tv,android,ios,web
 */
...
// Prefer TV/Android first
const YT_CLIENTS = (process.env.YT_CLIENTS || "tv,android,ios,web").split(",").map(s => s.trim()).filter(Boolean);

// Which clients *may* use cookies (only if cookieGood=true)
const COOKIE_CLIENTS = (process.env.COOKIE_CLIENTS || "web,tv,android,ios")
  .split(",").map(s => s.trim().toLowerCase()).filter(Boolean);

// Treat very small jars as “bad” to avoid triggering auth-challenges
const MIN_COOKIE_BYTES = Number(process.env.MIN_COOKIE_BYTES || 20000);
...
function decodeCookieSources() {
  const candidates = [];
  // PATH
  if (ENV_PATH && fs.existsSync(ENV_PATH)) {
    try { const t = fs.readFileSync(ENV_PATH, "utf8"); candidates.push({ name:"PATH", text:t, bytes:t.length, health:cookieHealth(t) }); }
    catch (e) { console.warn("Cookie PATH read failed:", e.message); }
  }
  // B64_GZ
  if (ENV_B64_GZ) {
    try { const t = zlib.gunzipSync(Buffer.from(String(ENV_B64_GZ).replace(/\s+/g,""),"base64")).toString("utf8"); candidates.push({ name:"B64_GZ", text:t, bytes:t.length, health:cookieHealth(t) }); }
    catch (e) { console.warn("Cookie B64_GZ decode failed:", e.message); }
  }
  // B64
  if (ENV_B64) {
    try { const t = Buffer.from(String(ENV_B64).replace(/\s+/g,""),"base64").toString("utf8"); candidates.push({ name:"B64", text:t, bytes:t.length, health:cookieHealth(t) }); }
    catch (e) { console.warn("Cookie B64 decode failed:", e.message); }
  }
  if (!candidates.length) return null;
  // Prefer ones with googleAuth; among those, pick largest
...
let COOKIE_GOOD = false;

try {
  if (!FORCE_NO_COOKIES) {
    const chosen = decodeCookieSources();
    if (chosen) {
      ...
      COOKIE_GOOD = !!(COOKIE_HEALTH.googleAuth && COOKIE_BYTES >= MIN_COOKIE_BYTES);
      if (!COOKIE_HEALTH.googleAuth) console.warn("⚠️ Cookie jar lacks Google auth cookies (SID/SAPISID family).");
      if (COOKIE_BYTES < MIN_COOKIE_BYTES) console.warn(`⚠️ Cookie jar looks small (<${MIN_COOKIE_BYTES}B). Export full profile cookies.`);
...
app.get("/cookiez", (_req, res) => {
  res.json({
    ...
    cookieGood: COOKIE_GOOD,
    minCookieBytes: MIN_COOKIE_BYTES,
...
// ========= Cache =========
const cache = new NodeCache({ stdTTL: INFO_TTL, checkperiod: Math.max(30, Math.min(INFO_TTL,120)), useClones:false });

// ========= Concurrency =========
let running = 0;
const queue = [];
function schedule(fn){return new Promise((resolve,reject)=>{queue.push(async()=>{running++;try{resolve(await fn());}catch(e){reject(e);}finally{running--;tick();}});tick();});}
function tick(){while(running<MAX_YTDLP && queue.length){const job=queue.shift(); if(job) job();}}

// ========= Helpers =========
const YT_ID = /^[\w-]{11}$/;
function setCacheHeaders(res, seconds, extra="public"){res.set("Cache-Control",`${extra}, max-age=${seconds}, s-maxage=${seconds}, stale-while-revalidate=${Math.ceil(seconds/2)}`);}
function setShortPrivate(res, seconds=60){res.set("Cache-Control",`private, max-age=${seconds}, stale-while-revalidate=${Math.ceil(seconds/2)}`);}
function computeUrlTtlSec(urlStr,{floor=30,ceil=3600,safety=30}={}){try{const u=new URL(urlStr);const exp=Number(u.searchParams.get("expire")||u.searchParams.get("Expires"));if(Number.isFinite(exp)&&exp>0){const now=Math.floor(Date.now()/1000);return Math.max(floor,Math.min(ceil,exp-now-safety));}}catch{} return URL_TTL_FALLBACK;}
const pickUrl = f => f?.url||f?.manifest_url||f?.hls_manifest_url||f?.dash_manifest_url||f?.fragment_base_url||null;
const looksLikeHls = (url,proto,ext)=> (typeof proto==="string" && proto.includes("m3u8")) || (typeof ext==="string" && ext.includes("m3u8")) || (typeof url==="string" && url.includes(".m3u8"));
const isStoryboard = f => f?.protocol==="mhtml" || /^sb\d/.test(String(f?.format_id||""));
function scoreInfo(info){const fmts=Array.isArray(info?.formats)?info.formats:[];const maxH=fmts.reduce((m,f)=>Math.max(m,Number(f?.height)||0),0);const adaptive=fmts.filter(f=>(f?.vcodec&&f.vcodec!=="none")&&(!f?.acodec||f.acodec==="none")).length;return maxH*1000+adaptive;}
function mergeInfos(infos){if(!infos.length) return {formats:[]};const out={...infos[0],formats:[]};const seen=new Set();for(const info of infos){const fmts=Array.isArray(info?.formats)?info.formats:[];for(const f of fmts){const url=pickUrl(f); if(!url) continue; const key=`${String(f?.format_id||"")}|${String(f?.ext||"")}|${String(f?.protocol||"")}|${String(f?.height||"")}|${url.slice(0,120)}`; if(seen.has(key)) continue; seen.add(key); out.formats.push(f);}} return out;}
const children = new Set(); function track(child){try{children.add(child.pid);}catch{} child.on("close",()=>{try{children.delete(child.pid);}catch{}}); return child;}

// ========= Error helpers =========
function isAuthWallError(msg=""){const s=String(msg);return /Sign in to confirm you.?re not a bot/i.test(s)||/This video may be inappropriate/i.test(s)||/account associated/i.test(s);}
function willUseCookiesFor(client, requestedUseCookies){return requestedUseCookies && HAS_COOKIES && COOKIE_GOOD && COOKIE_CLIENTS.includes(String(client||"").toLowerCase());}
...
function spawnYtDlpJSON(url, playerClient="tv", useCookies=true){
  return schedule(()=>new Promise((resolve,reject)=>{
    ...
    const args = ["-J","--no-warnings","--no-progress","--extractor-args",`youtube:player_client=${playerClient}`,"--socket-timeout",String(YTDLP_SOCKET_TIMEOUT_SEC),"--retries",String(YTDLP_RETRIES)];
    ...
    let out="", err=""; const timer=setTimeout(()=>{try{child.kill("SIGKILL");}catch{}}, YTDLP_TIMEOUT_MS);
    child.stdout.setEncoding("utf8"); child.stdout.on("data",c=>out+=c); child.stderr.on("data",c=>err+=c);
    ...
    child.on("close",code=>{
      ...
      if (!out && code!==0){
        ...
        if (isAuthWallError(em) && allowCookies){ console.warn(`⚠️ AUTH wall (JSON) client=${playerClient} with cookies → retrying without cookies`); spawnYtDlpJSON(url, playerClient, false).then(resolve).catch(reject); return; }
        const e=new Error(em); if (isAuthWallError(em)) e.code="AUTH_REQUIRED"; return reject(e);
      }
...
function spawnYtDlpBestUrl(url, playerClient="tv", useCookies=true){
  return schedule(()=>new Promise((resolve,reject)=>{
    const allowCookies = willUseCookiesFor(playerClient, useCookies);
    const args = ["-g","--no-warnings","--no-progress","--extractor-args",`youtube:player_client=${playerClient}`,"--socket-timeout",String(YTDLP_SOCKET_TIMEOUT_SEC),"--retries",String(YTDLP_RETRIES),"-f",'best[ext=mp4][acodec!=none][vcodec!=none]/best[acodec!=none][vcodec!=none]/best'];
    if (YTDLP_FORCE_IPV4) args.unshift("-4");
    if (allowCookies) args.push("--cookies", YT_COOKIES);
    args.push(url);
    ...
    child.on("close",code=>{
      ...
      if (code!==0){
        ...
        if (isAuthWallError(em) && allowCookies){ console.warn(`⚠️ AUTH wall (-g) client=${playerClient} with cookies → retrying without cookies`); spawnYtDlpBestUrl(url, playerClient, false).then(resolve).catch(reject); return; }
        const e=new Error(em); if (isAuthWallError(em)) e.code="AUTH_REQUIRED"; return reject(e);
      }
      const direct = out.trim().split("\n").pop(); if (!direct) return reject(new Error("No direct URL from yt-dlp -g"));
...
async function spawnBestUrlAnyClient(url, opts = {}){
  ...
  for (const client of clients){
    const willCookie = willUseCookiesFor(client, wantCookies);
    console.log(`Trying -g client=${client} cookies=${willCookie}`);
    try { const direct = await spawnYtDlpBestUrl(url, client, wantCookies); return { url: direct, client }; }
    catch(e){ if (e && e.code==="AUTH_REQUIRED") authHit=true; lastErr=e; }
  }
...
function buildFormats(info){
  ...
  const videoFormats = formats
    ...
    .map(f=>{
      const url=pickUrl(f); const height=Number(f?.height)||0; const hls=looksLikeHls(url,f?.protocol,f?.ext);
      return { format_id:f?.format_id, extension:f?.ext||(hls?"m3u8":"mp4"), resolution:height?`${height}p`:"unknown", height, protocol:f?.protocol||(hls?"m3u8_native":"https"), has_audio:hls?true:!!(f?.acodec&&f.acodec!=="none"), bandwidth:f?.tbr||f?.abr||null, url };
    })
    .filter(fmt=>fmt.url)
  ...
  const audioFormats = formats
    ...
    .map(f=>({ format_id:f?.format_id, extension:f?.ext||"m4a", protocol:f?.protocol||"https", bitrate:f?.abr||f?.tbr||null, url:pickUrl(f) }))
    .filter(fmt=>fmt.url)
...
async function fetchInfoMergedFast(id, opts = {}){
  ...
  const all = clients.map(client =>
    spawnYtDlpJSON(url, client, wantCookies).then(info=>{
      ...
      const usable = fmts.some(f=>pickUrl(f)); if(!usable) throw new Error("No usable formats");
      return { client, info, score: scoreInfo(info) };
    })
  );

  let first;
  try { first = await Promise.any(all); }
  catch (e) {
    try {
      const { url: directUrl, client: gClient } = await spawnBestUrlAnyClient(url, { clients, nocookie: opts.nocookie });
      return { info:{ id, formats:[{ format_id:"best", ext: directUrl.includes(".m3u8")?"m3u8":"mp4", protocol: directUrl.includes(".m3u8")?"m3u8":"https", vcodec:"unknown", acodec:"unknown", url: directUrl }] }, client:`best-url:${gClient}` };
    } catch (eg) {
...
// ========= Backpressure =========
function rejectIfBusy(res){ if (queue.length>MAX_QUEUE){ res.set("Retry-After","3"); res.status(503).json({error:"Busy, please retry"}); return true; } return false; }

// ========= Routes =========
app.get("/healthz", (_req,res)=>{ res.json({
  ok:true, running, queued:queue.length, maxYtDlp:MAX_YTDLP,
  cacheKeys:cache.keys().length, uptimeSec:Math.floor(process.uptime()),
  rssMB:Math.round(process.memoryUsage().rss/(1024*1024)),
  clients:YT_CLIENTS, cookieClients:COOKIE_CLIENTS,
  cookieSource:COOKIE_SOURCE, cookieBytes:COOKIE_BYTES,
  checks:COOKIE_HEALTH, cookieGood:COOKIE_GOOD, minCookieBytes:MIN_COOKIE_BYTES,
  forceNoCookies:FORCE_NO_COOKIES
});});

function parseClientListParam(q){ const raw=String(q||"").trim(); if(!raw) return null; const arr=raw.split(",").map(s=>s.trim()).filter(Boolean); return arr.length?arr:null; }
...
app.post("/prewarm", async (req,res)=>{
  try{
    ...
    if (e?.code==="AUTH_REQUIRED"){ return res.status(403).json({ error:"YouTube requires sign-in (anti-bot). Provide cookies.", code:"AUTH_REQUIRED", checks:COOKIE_HEALTH }); }
...
// ========= Listen =========
app.listen(PORT,"0.0.0.0", ()=>{ console.log(`✅ yt-dlp server running on http://localhost:${PORT}`); });
